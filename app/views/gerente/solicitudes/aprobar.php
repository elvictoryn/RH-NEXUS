...
if (!$solicitud || $solicitud['sede_id'] != $sede_id) {
    $_SESSION['error_decision'] = "Solicitud no encontrada o no tienes permisos para aprobarla";
    header('Location: lista.php');
    exit;
}
...
if ($solicitud['estado'] !== 'enviada a gerencia') {
    $_SESSION['error_decision'] = "Solo se pueden aprobar solicitudes enviadas a gerencia";
    header('Location: lista.php');
    exit;
}
...
try {
    ...
    // Cambiar estado a "aceptada gerencia"
    $resultado = $solicitud_model->cambiarEstado($solicitud_id, 'aceptada gerencia');
    
    if ($resultado) {
        // El trigger automáticamente:
        // 1. Registrará el cambio en el historial
        // 2. Notificará a RH del departamento
        
        $_SESSION['solicitud_aprobada'] = "Solicitud aprobada exitosamente. Se procede con el proceso de RH.";
        header('Location: lista.php');
        exit;
    } else {
        $_SESSION['error_decision'] = "Error al aprobar la solicitud";
        header('Location: lista.php');
        exit;
    }
    ...
    $_SESSION['error_decision'] = "Error interno del servidor al aprobar la solicitud";
    header('Location: lista.php');
    exit;
}
?> 