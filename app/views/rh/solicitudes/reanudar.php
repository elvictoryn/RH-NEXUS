...
// Verificar que la solicitud esté en un estado que permita reanudarla
$estados_permitidos = ['cerrada'];
if (!in_array($solicitud['estado'], $estados_permitidos)) {
    $_SESSION['error_decision'] = "Solo se pueden reanudar solicitudes cerradas";
    header('Location: lista.php');
    exit;
}
...
try {
    ...
    if ($resultado) {
        ...
        $_SESSION['proceso_reanudado'] = "Proceso de solicitud reanudado exitosamente. La solicitud está nuevamente en proceso de RH.";
        header('Location: lista.php');
        exit;
    } else {
        $_SESSION['error_decision'] = "Error al reanudar el proceso de la solicitud";
        header('Location: lista.php');
        exit;
    }
    
} catch (Exception $e) {
    error_log("Error al reanudar proceso de solicitud: " . $e->getMessage());
    $_SESSION['error_decision'] = "Error interno del servidor al reanudar el proceso";
    header('Location: lista.php');
    exit;
}
?>
