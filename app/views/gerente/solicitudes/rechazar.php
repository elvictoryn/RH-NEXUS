...
if (!isset($_GET['motivo']) || empty(trim($_GET['motivo']))) {
    $_SESSION['error_decision'] = "Motivo del rechazo no proporcionado";
    header('Location: lista.php');
    exit;
}

$motivo_rechazo = trim($_GET['motivo']);

// Validar longitud del motivo
if (strlen($motivo_rechazo) < 10) {
    $_SESSION['error_decision'] = "El motivo del rechazo debe tener al menos 10 caracteres";
    header('Location: lista.php');
    exit;
}
...
if (!$solicitud || $solicitud['sede_id'] != $sede_id) {
    $_SESSION['error_decision'] = "Solicitud no encontrada o no tienes permisos para rechazarla";
    header('Location: lista.php');
    exit;
}
...
if ($solicitud['estado'] !== 'enviada a gerencia') {
    $_SESSION['error_decision'] = "Solo se pueden rechazar solicitudes enviadas a gerencia";
    header('Location: lista.php');
    exit;
}
...
try {
    ...
    $resultado = $solicitud_model->cambiarEstado($solicitud_id, 'rechazada', $motivo_rechazo);
    ...
    if ($resultado) {
        ...
        $_SESSION['solicitud_rechazada'] = "Solicitud rechazada exitosamente. El jefe de área será notificado.";
        header('Location: lista.php');
        exit;
    } else {
        $_SESSION['error_decision'] = "Error al rechazar la solicitud";
        header('Location: lista.php');
        exit;
    }
    ...
    $_SESSION['error_decision'] = "Error interno del servidor al rechazar la solicitud";
    header('Location: lista.php');
    exit;
}
?> 